...
<div class="mt-4">
    ...
    @enderror
        ...
        <!-- Payroll -->
      <div class="mt-4">
        <label for="mode" class="form-label">Payroll</label>
        <select name="name" class="form-select">
        @foreach ($payrollperiods as $payrollperiod)
            <option value="{{ $payrollperiod->id }}" {{ old('name', $selectedEmployeeId ?? '') == $employee->id ? 'selected' : '' }}>
      ...
      <!-- Adjustment Type -->
      <div class="mt-4">
    <label for="name" class="form-label">Adjustment Type</label>
    <select name="name" class="form-select">
    ...
    @enderror
      ...
      <!-- Adjustment Amount -->
        <div class="mt-4">
        <label for="mode" class="form-label">Adjustment Amount</label>
        <input type="number" id="mode" name="mode" value="{{ old('mode')}}" class="form-control"  autofocus autocomplete="mode" />
        @error('name')
          <div class="mt-2 text-danger">{{ $message }}</div>
        @enderror
      </div>
